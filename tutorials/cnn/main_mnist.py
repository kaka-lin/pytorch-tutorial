...
def main(batch_size, epochs, lr,
    ...
    if not testMode:
        for epoch in range(0, epochs):
            ...
            train_model(model, train_loader, optimizer, loss, batch_size, isCUDA)
        ...
        if saveModel:
            ...
            save_model(model, mode=save_model_mode, model_name=model_name, **checkpoint)
    else:
        # Load model
        model = CNN()
        if isCUDA:
        ...
        checkpoint = torch.load('./models/pre_trains/binary_model_checkpoint.tar')
